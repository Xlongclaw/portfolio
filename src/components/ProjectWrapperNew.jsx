...
const ProjectWrapperNew = ({ project }) => {
  return (
    <div className="w-[100%] sm:w-[24rem] flex flex-col gap-2">
      <img
        className="w-full sm:h-3/4 h-[50vw] object-cover object-left-top border border-WHITE/10"
        src={project.image}
        alt=""
      />
      <div className="w-full flex sm:flex-row flex-col items-center sm:gap-1 gap-2">
        <h3 className="border border-WHITE/10 sm:w-[13rem] w-full sm:flex-1 sm:h-[3.5rem] h-[3rem] flex justify-center items-center text-xs">
          {project.name.toUpperCase()}
        </h3>
        <div className="flex sm:h-[3.5rem] h-[3rem] w-full gap-1">
